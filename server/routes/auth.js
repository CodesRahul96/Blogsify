...
// Verify 2-Step Verification TOTP code or Backup Recovery Code during Login
router.post("/verify-2fa", async (req, res) => {
  try {
    ...
    if (!isSuccess && user.twoFactorRecoveryCodes && user.twoFactorRecoveryCodes.length > 0) {
      const recoveryIndex = user.twoFactorRecoveryCodes.indexOf(cleanCode);
      if (recoveryIndex !== -1) {
        // Burn this recovery code so it cannot be used again
    ...
    if (!isSuccess) {
      return res.status(401).json({ message: "Invalid verification code or recovery key." });
    }
...
router.post("/2fa/setup", auth, async (req, res) => {
  try {
    ...
    // Save as PENDING secret so active 2FA is never broken if user cancels setup
...
// 2FA Verify & Activate: Confirm the code generated by the Authenticator app & generate recovery codes
router.post("/2fa/verify-setup", auth, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.id);
    const candidateSecret = user?.twoFactorPendingSecret || user?.twoFactorSecret;
    ...
    // Generate 8 cryptographically secure 8-character recovery backup codes
    const recoveryCodes = Array.from({ length: 8 }, () =>
      crypto.randomBytes(4).toString("hex").toUpperCase()
    );

    // Mark 2FA as officially active and save secret + recovery codes
...
// 2FA Disable: Require user password to disable 2FA
router.post("/2fa/disable", auth, async (req, res) => {
  try {
    const { password } = req.body;
    if (!password || typeof password !== "string") {
      return res.status(400).json({ message: "Your current account password is required to disable 2FA" });
    }
...
router.get("/me", auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select(
      "-password -twoFactorSecret -twoFactorPendingSecret -twoFactorRecoveryCodes -twoFactorCode"
    );
    if (!user) return res.status(404).json({ message: "User not found" });
    res.json(user);
  } catch (err) {
